  ...
  if (Array.isArray(referenceImages) && referenceImages.length > 0) {
    ...
    for (const r of referenceImages) {
      if (!r || typeof r !== 'object') {
        return res.status(400).json({ success: false, error: 'referenceImages: cada item deve ter { url, mode }' });
      }
      if (!isInternalUploadUrl(r.url)) {
        return res.status(400).json({ success: false, error: `URL de referência inválida: ${r.url}` });
      }
      const mode = VALID_MODES.includes(r.mode) ? r.mode : 'inspiration';
      refMetadata.push({ url: r.url, mode });
      refUrls.push(r.url);
    }
