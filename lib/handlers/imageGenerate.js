...
async function imageGenerateHandler(req, res) {
  ...
  const {
    ...
    bypassCache,
    // sprint Image v2 (maio/2026): operador escolheu templates da Arte Guia
    inspirationTemplateIds,
  } = req.body || {};
  ...
  if (Array.isArray(referenceImages) && referenceImages.length > 0) {
    ...
    for (const r of referenceImages) {
      ...
      // sprint Image v2: preserva templateId/templateScope no metadata
      // quando a ref veio do picker da Arte Guia. Worker usa pra carregar
      // ai_description e injetar no Prompt Engineer.
      const meta = mode ? { url: r.url, mode } : { url: r.url };
      if (r.templateId && r.templateScope) {
        meta.templateId = r.templateId;
        meta.templateScope = r.templateScope;
      }
      refMetadata.push(meta);
      refUrls.push(r.url);
    }
  ...
  // sprint Image v2 (maio/2026): incrementa usage_count dos templates da
  // Arte Guia escolhidos. Fire-and-forget — nao bloqueia a resposta.
  if (Array.isArray(inspirationTemplateIds) && inspirationTemplateIds.length) {
    setImmediate(async () => {
      try {
        const { incrementUsageCount } = require('../../models/inspirationTemplate.model');
        for (const t of inspirationTemplateIds) {
          if (t?.id && t?.scope) {
            await incrementUsageCount(t.id, t.scope, tenantId);
          }
        }
      } catch (err) {
        console.warn('[WARN][API:image/generate] incrementUsageCount falhou', { error: err.message });
      }
    });
  }
