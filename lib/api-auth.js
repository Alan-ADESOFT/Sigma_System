...
/**
 * Exige usuário autenticado E com role admin/god.
 * Atalho: chama requireAuth + valida isAdmin numa única expressão.
 *
 * @param {object} req
 * @returns {Promise<object>} user autenticado e admin
 * @throws {Error} statusCode=401 se não autenticado, 403 se autenticado mas
 *                não-admin.
 */
async function requireAdmin(req) {
  const user = await requireAuth(req);
  if (!isAdmin(user)) {
    const err = new Error('Apenas admin ou god pode realizar essa ação');
    err.statusCode = 403;
    throw err;
  }
  return user;
...
module.exports = { requireAuth, requireAdmin, isAdmin, handleAuthError };
